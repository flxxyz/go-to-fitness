    ...
    return (
      <View className='picker-seleter' >
        ...
        <View className='picker-control' >
          <Text className='picker-control-button'
            onClick={this.onCancel}>取消</Text>
          <Text className='picker-control-button'
            style="float: right"
            onClick={this.onSuccess}>确定</Text></View>
        <PickerView className='picker' value={value}
          onChange={this.onChange} >
          <PickerViewColumn className="picker-column" > {
            years.map(item => {
              return (<View className='picker-column-item' key>{item}年</View>
              )
            })
          }</PickerViewColumn>
          <PickerViewColumn className="picker-column">
          ...
          <PickerViewColumn className="picker-column">
            {
              daysData.map(item => {
                if (!item.isPlaceholder) {
                  return (
                    <View className='picker-column-item' key>
                      {item.day}日
              </View>
                  )
                }
